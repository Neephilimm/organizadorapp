...
-- Conexión a Canvas LMS (dominio + token personal, uno por usuario ya que
-- cada universidad tiene su propio Canvas)
...
create policy "canvas_tokens_propios" on canvas_tokens
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
